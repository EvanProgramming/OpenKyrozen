...
安装器会获取不可变的 GitHub `v2.0.4` 发布 wheel，检查操作系统、架构、Python、网络和用户目录权限；需要时安装 `uv`；在隔离的 `uv` 工具环境中配置 Web 依赖；创建私有 `~/.kyrozen` 状态目录；并验证 `kyrozen --version` 与 `kyrozen --help`。安装器不会读取、打印或上传 API 密钥，首次运行 `kyrozen` 时再引导服务商配置。
...
v2.0.4 发布 wheel 已由 GitHub Actions 构建并验证，详见 [v2.0.4 发布版](https://github.com/EvanProgramming/OpenKyrozen/releases/tag/v2.0.4)。安装后可在任意调用目录运行 `kyrozen` 和 `kyrozen-web`。加密的服务商配置保存在 `~/.kyrozen_config.json`。
...
| `/update` | 使用固定的 GitHub `v2.0.4` wheel 就地更新已安装工具；成功后重启 Kyrozen（不会向项目执行 git pull） |
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│   工具执行器    │──► 39 项运行时工具（文件 I/O、Shell、Git、网页、记忆、浏览器、图谱、GitHub）
└────────┬────────┘
         │  工具结果反馈给 LLM
         │  （每轮最多 50 次工具调用）
         ▼
...
所有 39 项运行时工具在 JSON 动作块中接受纯字符串 `args` 字段。完整的
...
| `POST` | `/api/v2/learning/provider` | 选择本地或远程学习运行时 |
...
https://github.com/EvanProgramming/OpenKyrozen/releases/download/v2.0.4/openkyrozen-2.0.4-py3-none-any.whl &&
...
已发布的 `v2.0.4` 版本不可变，公共安装器和 `/update` 会固定到该版本。
